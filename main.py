...
class GoogleKeywordScreenshooter:
    ...
    def start(self):
        ...
        try:
            shitty_element = WebDriverWait(self.browser, 6).until(
                EC.presence_of_element_located((By.CLASS_NAME, "g-blk"))
            )
            self.browser.execute_script(
                """
            const shitty = arguments[0,1];
...
domain_competitors = GoogleKeywordScreenshooter("tesla", "screenshots")
domain_competitors.start()
domain_competitors.finish()
python_competitors = GoogleKeywordScreenshooter("apple", "screenshots")
